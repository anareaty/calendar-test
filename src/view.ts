...
export default class CalendarView extends ItemView {
  ...
  public revealActiveNote(): void {
    const { moment } = window;
    const { activeLeaf } = this.app.workspace as any;
  ...
  async openOrCreateDailyNote(
    ...
    if (!existingFile) {
      ...
      // Try to find monthly note

      let monthlyNote = getMonthlyNote(date, this.plugin.options)
      ...
      if (monthlyNote) {
          ...
          await leaf.openLinkText(linkText, "", { mode })
